...
export default function DriverDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(`http://127.0.0.1:5000/api/find-parking`)
          ...
          <div className="card-body">
            <div className="flex flex-wrap items-center justify-between mb-4">
              ...
              <div className="flex space-x-2 mt-2 sm:mt-0">
                <div className="join">
                  <button 
                    className={`join-item btn btn-sm ${timeOfDay === 'morning' ? 'btn-active' : ''}`}
                    onClick={() => handleTimeChange('morning')}
                  >
                    Morning
                  </button>
                  <button 
                    className={`join-item btn btn-sm ${timeOfDay === 'afternoon' ? 'btn-active' : ''}`}
                    onClick={() => handleTimeChange('afternoon')}
                  >
                    Afternoon
                  </button>
                  <button 
                    className={`join-item btn btn-sm ${timeOfDay === 'evening' ? 'btn-active' : ''}`}
            ...
            <div className="flex flex-wrap items-center justify-between mb-4">
              <div className="tabs tabs-boxed bg-gray-700 mb-2">
                <a 
                  className={`tab ${mapMode === 'both' ? 'bg-emerald-500 text-white' : 'text-gray-300'}`}
                  onClick={() => setMapMode('both')}
                >
                  All Layers
                </a>
                <a 
                  className={`tab ${mapMode === 'hotspots' ? 'bg-emerald-500 text-white' : 'text-gray-300'}`}
                  onClick={() => setMapMode('hotspots')}
                >
                  Hotspots Only
                </a>
                <a 
                  className={`tab ${mapMode === 'parking' ? 'bg-emerald-500 text-white' : 'text-gray-300'}`}
            ...
            <div className="mt-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              {data.hotspots && data.hotspots.length > 0 && (
                <div className="alert bg-gray-700 border border-gray-600 flex-1">
                  ...
                  <span className="text-gray-200">
                    Top hotspot: <span className="text-emerald-400 font-bold">{getTopHotspot()}</span> (Time: {getHourFromTime(timeOfDay)})
                  </span>
                </div>
              )}
              ...
              {data.parking_spots && data.parking_spots.length > 0 && (
                <div className="alert bg-gray-700 border border-gray-600 flex-1">
                  ...
                  <span className="text-gray-200">
                    Found <span className="text-blue-400 font-bold">{data.parking_spots.length}</span> parking spots
